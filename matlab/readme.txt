    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_RDHDF.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * RDHDF.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * RDHDF.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * RDHDF (application created by MATLAB Compiler)
        * run_RDHDF.sh (shell script that can be run to 
    ...
    All Platforms:
        * RDHDF.dll (shared library, file extension varies by 
          platform)
        * RDHDF.h (library header file)
        * RDHDF.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * RDHDF.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * RDHDF.xml (documentation files)
        * RDHDF.pdb (program database file - if DEBUG option 
          selected)
        * RDHDF.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * RDHDF.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * RDHDF.ctf (Component Technology File archive, contains 
          compiled M-code)
        * RDHDF.xla (Optionally created and placed in
